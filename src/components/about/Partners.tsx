...
const Partners = () => {
    return (
        <div className='w-full h-[calc(100vh-2rem)] lg:h-[calc(70vh-2rem)] bg-cover bg-center bg-no-repea bg-[url("/assets/hero2-img.png")]'>
            <div className='bg-black/40 w-full h-full py-12 px-6 pr-9 md:px-24 xl:px-60 flex flex-col items-center justify-center gap-8 lg:gap-24'>
                <motion.div className='flex flex-col items-center justify-center' initial={{ opacity: 0, y: -30 }}
                    whileInView={{ opacity: 1, y: 0 }}
                    viewport={{ once: false }}
                    transition={{ duration: 0.5 }}>
                    <p className='text-sm lg:text-base text-gray-900 px-4 py-3 shadow-lg rounded-full w-fit flex items-center justify-center gap-2 mb-5 md:mb-6 lg:mb-8 bg-white'><span className='w-3 h-3 lg:w-4 lg:h-4 bg-green-700 rounded-full p-1 text-black flex items-center justify-center shadow-md'>.</span>Certif & Partners<span className='w-3 h-3 lg:w-4 lg:h-4 bg-green-700 rounded-full p-1 text-black flex items-center justify-center shadow-md'>.</span></p>
                    <h1 className='font-bold text-2xl sm:text-3xl md:text-4xl xl:text-5xl text-gray-200'><span className='text-green-500 mb-2'>Certifications</span> & Partners</h1>
                    <p className='text-center mt-4 lg:mt-6 sm:text-lg text-gray-200'>E-SET GLOBAL SOLUTION SERVICES LIMITED is a legally registered company with a Certificate of Incorporation in Nigeria. We collaborate with leading industry partners to enhance our service delivery. <span className='font-semibold'>Some of them includes:</span>
                    </p>
                </motion.div>
                <div className='flex flex-wrap items-center justify-center gap-6'>
                    ...
                    <PartnerList text="ACHIEF ELECTRICAL" className='md:bg-green-700 md:text-gray-100 hover:bg-white hover:text-green-700' />
                    <PartnerList text="AFENG SOLUTIONS GLOBAL" className='md:bg-white hover:bg-green-700 hover:text-white md:text-green-700 ' />
