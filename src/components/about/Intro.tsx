...
const Grid = ({ img, title, text }: GridProps) => {
    return (
        ...
        >
            ...
            <div>
                <h3 className='font-semibold text-[14px] lg:text-[15px] mb-1 text-gray-900'>{title}</h3>
        ...
        <motion.div className='bg-white min-h-screen' initial={{ opacity: 0, y: 40 }}
            ...
            <div className='w-full h-full flex flex-col justify-center py-12 md:py-20 px-6 pr-9 md:px-24 xl:px-60 gap-10 lg:gap-20'>
                <div className='flex flex-col justify-center gap-4 lg:gap-6'>
                    <div className='flex flex-col lg:flex-row justify-start items-center lg:items-start lg:justify-evenly gap-6 lg:gap-8 w-full'>
                        <motion.div className='w-full xl:w-1/2' initial={{ opacity: 0, y: 20 }}
                            whileInView={{ opacity: 1, y: 0 }}
                            transition={{ duration: 0.6 }}
                            viewport={{ once: false }}>
                            <p className='text-[10px] sm:text-sm lg:text-base text-gray-900 px-4 py-3 shadow-lg rounded-full w-fit flex items-center justify-center gap-2 mb-5 md:mb-6 lg:mb-8 bg-white'><span className='w-3 h-3 lg:w-4 lg:h-4 bg-green-700 rounded-full p-1 text-black flex items-center justify-center shadow-md'>.</span>About Us<span className='w-3 h-3 lg:w-4 lg:h-4 bg-green-700 rounded-full p-1 text-black flex items-center justify-center shadow-md'>.</span></p>
                            <h1 className='font-bold text-2xl sm:text-3xl lg:text-4xl text-gray-900'>Meet E-SET <span className='text-green-700'>GLOBAL</span> <br /> <span className='font-semibold text-xl sm:text-xl xl:text-2xl'>Your Solar Partner</span></h1>
                        </motion.div>
                        <div className='text-wrap flex flex-col gap-2 md:gap-4 text-gray-900 lg:text-lg xl:w-1/2'>
                ...
                <div className='hidden xl:block'>
                    <motion.div className='relative w-full h-[500px] flex items-center' initial={{ opacity: 0, scale: 0.95 }}
                        ...
                        <div className='absolute bg-white p-2 translate-x-[70%] rounded-4xl shadow-2xl'>
